...
def upgrade() -> None:
    # Create forum_posts table with CHECK constraint instead of enum type
    # This avoids enum management complexity and migration issues
    op.create_table(
        "forum_posts",
        ...
        sa.CheckConstraint(
            ...
            name="ck_forum_posts_moderation_status"
        ),
    )
    ...
    # Create forum_replies table with CHECK constraint
    op.create_table(
        "forum_replies",
        sa.Column("id", sa.String(36), primary_key=True),
        ...
        sa.CheckConstraint(
            "moderation_status IN ('pending', 'approved', 'rejected', 'flagged')",
            name="ck_forum_replies_moderation_status"
        ),
    )
